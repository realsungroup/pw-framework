...
import { message, DatePicker, Form, Input, Button, Table, Select } from 'antd';
...
class LzStepsMAP extends React.Component {
    ...
    return (
      ...
      >
        <Form>
          ...
          <FormItem label="加班类型">
            {getFieldDecorator('C3_593178257671', {
              rules: [{ required: true, message: '时长' }]
            })(
              <Select>
                <Option value="平时加班">平时加班</Option>
                <Option value="工休加班">工休加班</Option>
          ...
          <FormItem label="加班事由">
            {getFieldDecorator('C3_593375352611', {
              rules: [{ required: false, message: '加班事由' }]
            })(<TextArea />)}
          </FormItem>
        </Form>
      </div>
  ...
  getParams = values => {
    ...
    const params = personList.map((person, index) => ({
      ...
      C3_489231991382: values.C3_489231991382.format('YYYY-MM-DD HH:mm'), // 开始时间
      C3_489231991601: values.C3_489231991601.format('YYYY-MM-DD HH:mm'), // 结束时间
      C3_489232060991: values.C3_489232060991, // 小时
      C3_593375352611: values.C3_593375352611, // 加班事由
      C3_593178257671: values.C3_593178257671, // 加班类型
      map_id: person.mapId
    }));
    return params;
  };
