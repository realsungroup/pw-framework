...
class LzTable extends React.Component {
  static propTypes = {
    ...
    /**
     * 调用 api（http://kingofdinner.realsun.me:8102/api/Resource/RunAutoImportByName?title=测试审批） 进行批量处理的按钮的配置
     * 默认：-
     */
    batchBtn: PropTypes.bool
    // {
    //   btnName: '批量按钮', // 按钮名称
    //   title: '测试审批' // api 的 title 查询参数
    // }
  };
  static defaultProps = {
    dataMode: 'main',
  ...
  getButtons = async () => {
    ...
    function isRowSelection(backEndBtnsMultiple) {
      const popNumArr = [6, 7, 8];
      const arr = backEndBtnsMultiple.filter(
        item => popNumArr.indexOf(item.Type) === -1
      );
      return !!arr.length;
    }
    ...
    if (backEndBtnsMultiple.length && isRowSelection(backEndBtnsMultiple)) {
      rowSelection = {
        columnWidth: 60,
  ...
  saveMultipleRecord = async () => {
    ...
    pagination = pagination ? (pagination.current = 0) : null;
    this.setState({
      pagination,
  ...
  handleTableChange = async (pagination, filters, sorter, extra) => {
    ...
    await this.getTableData(o, this.wheres);
    this.setState({ loading: false });
  };
  ...
  renderRowCustomBtns = (record, rowIndex, customBtns) => {
    return customBtns.map(customBtn => {
      // 带有 Popconfirm 组件的自定义按钮
      if (customBtn.popConfirmProps) {
        return (
          <Popconfirm
            {...customBtn.popConfirmProps}
            onConfirm={() => customBtn.onClick(record, this.refreshTableData)}
          >
    ...
    return (
      <Fragment>
        {backEndBtns.map(btnInfo => {
          let Url = btnInfo.Url;
          let hasQS = Url.indexOf('?') !== -1;
          const concatString = hasQS ? '&' : '?';
          const url = `${btnInfo.Url}${concatString}resid=${resid}&subresid=${subresid}&hostrecid=${hostrecid}&subrecid=${record.REC_ID}&token=${token}&searchvalue=${searchValue}`;
          return (
            <LzBackendBtn
              key={btnInfo.Name1}
              btnInfo={btnInfo}
  ...
  renderBatchBtn = () => {
    const { batchBtn } = this.props;
    return (
      <Popconfirm
        title={batchBtn.popconfirmTitle || '您确定要操作吗？'}
        okText="确定"
        cancelText="取消"
        onConfirm={this.handleBatchBtnClick}
      >
        <Button size="small">{batchBtn.btnName}</Button>
      </Popconfirm>
    );
  };

  handleBatchBtnClick = async () => {
    const { batchBtn } = this.props;
    ...
    if (!taskId) {
      return message.error('操作失败！无 taskId');
    } else {
      message.success('操作成功');
    }
    this.setState(
      { batchDealLoading: true, curIndex: 0, totalIndex: 0 },
      () => {
    ...
    switch (unitBtnInfo.name) {
      ...
      case 'LzPersonCA': {
        return (
          <Fragment>
            {unitModalVisible && (
              <LzModal onClose={this.onCloseLzModal} defaultScaleStatus="max">
                <LzPersonCA record={record} {...unitBtnInfo.props} />
              </LzModal>
            )}
          </Fragment>
        );
  ...
  renderBatchProgress = () => {
    const { batchDealLoading, totalIndex, curIndex, curTaskTitle } = this.state;
    let percent = 0;
    if (totalIndex) {
    ...
    return (
      <div className={classNames('lz-table', className)} style={style}>
        <Spin spinning={loading}>
          ...
          {/* 后端按钮栏 */}
          {(hasBackendBtns || !!batchBtn) && (
            <div className="lz-table-op" style={lzTableOpStyle}>
              {this.renderBackendBtnsMultiple()}
              {!!batchBtn && this.renderBatchBtn()}
            </div>
          )}
