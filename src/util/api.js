...
// 添加记录(EditOrAdd)
export const addRecordEditOrAdd = (resid, data) => {
  const url = baseUrl + appConfig.path.saveData;
  const params = {
    resid,
    data
  };
  return dealNextExtractData(http(url, POST, params, dataType.EditOrAdd));
};
...
// 修改记录
export const modRecordEditOrAdd = (resid, data) => {
  const url = baseUrl + appConfig.path.saveData;
  const params = {
    resid,
    data
...
// 重置密码
export const resetPassByEmail = (email, enterprisecode) => {
  const url = baseUrl + 'api/Account/ForgetPassByEmail';
  const params = {
    email,
    enterprisecode
  };
 ...
 * @param {number} refresh 是否刷新日报：0 表示不刷新；1表示刷新；默认值为 0，不刷新
 */
export const getDailyRpt = (pnid, startDate, endDate, refresh = 0) => {
  ...
  const params = {
    ...
    date2: endDate,
    cmscolumns:
      'DATES,C3_375377576828,WORKSTARTTIME,WORKENDTIME,C3_375377591046',
    refresh
  };
  return dealNextExtractData(http(url, GET, params, dataType.UnKnow));
};
 ...
 * @param {string} endDate 结束日期；如：2018-10-30
 */
export const getDailyRptV2 = (pnid, startDate, endDate) => {
  const url = baseUrl + 'api/200/Attendance/GetEmpCalendar';
  const params = {
    pnid,
    date1: startDate,
    date2: endDate
