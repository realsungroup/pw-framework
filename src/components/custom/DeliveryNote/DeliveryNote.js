...
class DeliveryNote extends React.Component {
  ...
  handlePrintDel = () => {
    ...
    let style =
      '<style>.pd{padding:1rem;} .pd *{font-size:14px;margin:0!important;padding:0!important;box-sizing:border-box!important;} th{padding:0 .5rem;line-height: 1.4rem;height: 1.4rem;}.ant-calendar-picker i{display:none;}.ant-calendar-picker input{border:none;} input{background:rgba(0,0,0,0)!important;border:0px solid #fff!important;} </style>';
    ...
    return (
     ...
     <div className="wrap">
       ...
       <Button onClick={()=>{
        ...
        <div id='toPrintDel' style={{width:'794px',overflow:'hidden',height:'462px',marginLeft:'calc(50% - 397px)'}}>
          <div className='pd'>
          ...
          <div style={{overflow:'hidden'}}>
            ...
            <div style={{float:'left',textAlign:'center',width:'90%'}}>
              <h2 style={{fontSize:'18px'}}>无锡美银精工磨具有限公司</h2>
              <p>地址：无锡国家高级技术产业区（开元工业园）锡士路18号</p>
              <p>电话：0510-85346588 85342958 传真：0510-85346598 QQ：513405811</p>
            </div>
           </div>
        </div>
        <div style={{overflow:'hidden',textAlign:'left',marginTop:'.2rem'}}>
              ...
              <div style={{float:'left',width:'40%',textIndent:'16%'}}>
                <big style={{fontSize:'18px',fontWeight:'bold'}}>送货单</big>
              </div>
              <div style={{float:'left',width:'20%'}}>
                <p>送货单号：{this.state.C3_684709750566}</p>
                <p>日期：
                {/* <span className='hid'>{this.state.dateStr}</span> */}
                <span className='dateP' style={{overflow:'hidden'}}>
                <DatePicker
                 className='dateP'
