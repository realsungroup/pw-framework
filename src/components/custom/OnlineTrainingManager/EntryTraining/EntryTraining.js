...
class EntryTraining extends React.Component {
  ...
  /**
   * 根据C3_636218595792对课程排序
   */
  sortCourse = (courses = []) => {
    return courses.sort((a, b) => {
      return a.C3_636218595792 - b.C3_636218595792;
    });
  };

  filterCourses = memoize((courses = [], filterText = '') => {
    if (!filterText) {
      return this.sortCourse(courses);
    }
    ...
    return this.sortCourse(filterCourses);
  });

  /**
   * 移动课程
   * @param {Object} cousre
   * @param {number} direction 0:向下;1:向上
   */
  moveCourse = async (course, direction) => {
    const { courses } = this.state;
    const { baseURL } = this.props;
    const sourceNo = Number(course.C3_636218595792);
    let targetNo;
    if (direction === 1) {
      //向上
      targetNo = sourceNo - 1;
    ...
    const sourceIndex = courses.findIndex(
      course => course.C3_636218595792 == sourceNo
    );
    const targetIndex = courses.findIndex(
      course => course.C3_636218595792 == targetNo
    );
    const source = courses[sourceIndex];
    const target = courses[targetIndex];
    source.C3_636218595792 = targetNo;
    target.C3_636218595792 = sourceNo;
    try {
      await http({ baseURL }).modifyRecords({
        resid,
        data: [source, target]
      });
      courses[sourceIndex] = target;
      courses[targetIndex] = source;
  ...
  onDragEnd = async result => {
    ...
    const { courses } = this.state;
    const source = courses[result.source.index];
    const target = courses[result.destination.index];
    const targetNo = target.C3_636218595792;
    const sourceNo = source.C3_636218595792;
    source.C3_636218595792 = targetNo;
    target.C3_636218595792 = sourceNo;
    try {
      await http({ baseURL }).modifyRecords({
                      ...
                      >
                        {(provided, snapshot) => (
                          <div
                            ...
                            className={classnames('entry-training-course', {
                          ...
                          >
                            <span className="course-number">
                              {course.C3_636218595792}
                            </span>
                            <span>{course.C3_636218615452}</span>
                            <Popover
                              placement="rightTop"
                              content={
                                <div className="course-more-actions">
                                  {index !== 0 && (
                                    <div
                                      ...
                                      onClick={e => {
                                        e.stopPropagation();
                                        this.moveCourse(course, 1);
                                      }}
                                    >
                                      上移
                                    </div>
                                  )}
                                  {index !== _courses.length - 1 && (
                                    <div
                                      className="action"
                                      onClick={e => {
                                        e.stopPropagation();
                                        this.moveCourse(course, 0);
                                      }}
                                    >
                                      下移
                                    </div>
