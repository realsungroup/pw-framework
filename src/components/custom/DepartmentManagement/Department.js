...
class Department extends React.Component {
    ...
    if (calcNode.length) {
      let filterNodes = allNodes.filter(item => {
        ...
        if (item.tags.includes(selected)) {
          item.tags.splice(
            item.tags.findIndex(tag => tag === selected),
            1
          );
        }
   ...
   */
  getAllOrgs = () => {
    const url = `${this.baseURLAPI}api/v1/getAllOrgs`;
    fetch(url)
      .then(response => {
        return response;
      })
  ...
  renderHeader = () => {
    ...
    const { hasOpration, hasGroup, hudongSpecial } = this.props;

    return (
      <header className="department-chart_header">
        {mode === 'chart' && hasGroup && hudongSpecial && (
...
const composedHoc = compose(injectIntl, withImport, withModalDrawer());
