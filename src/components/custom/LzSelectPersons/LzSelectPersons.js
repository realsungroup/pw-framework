...
export default class LzSelectPersons extends React.Component {
  ...
  onChange = e => {
    ...
    if (value === '按部门添加') {
      ...
      // 主表数据
      const itemConfig = mainSubConfig.find(
  ...
  getMainTableData = async itemConfig => {
    let res;
    try {
      res = await getMainTableData(itemConfig.resid);
    } catch (err) {
      this.setState({ typeLoading: false });
  ...
  handleItemSelect = async (item, index) => {
    ...
    this.getPersonList(
      selectedItemConfig.resid,
      selectedItemConfig.subResid,
      item.REC_ID,
      option
    );
