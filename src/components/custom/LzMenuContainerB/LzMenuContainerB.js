...
import './LzMenuContainerB.less';
import LzPersonCAB from '../../../product-components/att/LzPersonCAB';
 ...
 * 菜单容器组件
 */
export default class LzMenuContainerB extends React.Component {
  static propTypes = {
    /**
     * 模式
  ...
  renderContent = () => {
    ...
    if (!record) {
      ...
      if (mode === 'single') {
        ...
        const { record } = this.state;
        console.log('ID', record.REC_ID);
        const props = {
          ...defaultComponetProps,
          hostrecid: record.REC_ID,
          searchValue: this.state.searchValue
        };
        return <LzPersonCAB {...props} key={props.hostrecid} />;
      } else {
